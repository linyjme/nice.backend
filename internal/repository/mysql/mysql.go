...
func New() (Repo, error) {
	cfg := global.NiceConfig.Mysql
	dbr, err := dbConnect(cfg.Read.Username, cfg.Read.Password, cfg.Read.Path, cfg.Read.Dbname)
	if err != nil {
		return nil, err
	}

	dbw, err := dbConnect(cfg.Write.Username, cfg.Write.Password, cfg.Write.Path, cfg.Write.Dbname)
	if err != nil {
		return nil, err
	}
...
func dbConnect(user, pass, addr, dbName string) (*gorm.DB, error) {
	...
	cfg := global.NiceConfig.Mysql.Base
